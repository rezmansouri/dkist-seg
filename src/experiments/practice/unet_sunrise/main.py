...
from sunrise import segDataset, segDataset_val, mIoULoss, FocalLoss, acc, eval_metrics_sem
...
def main():
    ...
    loss_fn = mIoULoss(weight=weights, n_classes=5).to(device) #nn.CrossEntropyLoss(weight=weights).to(device)#mIoULoss(weight=weights, n_classes=5)#FocalLoss(gamma=10, alpha=weights)
    ...
    for epoch in range(1, n_epochs+1):
        ...
        if is_best:
            ...
            ), '/home/dkist/out/experiments/unet_sunrise/model_iou/unet_epoch_{}_{:.5f}.pt'.format(epoch, np.mean(val_loss_list)))
    ...
    np.save('/home/dkist/out/experiments/unet_sunrise/stats_iou/stats.npy',
