...
class MitoDataset(Dataset):
    def __init__(self, root, start, end, size=(128, 128)):
        super(MitoDataset, self).__init__()
        images = []
        masks = []
        subdirs = sorted([s for s in os.listdir(root) if s.endswith('_2D')])
        for subdir in subdirs[start:end]:
            for image_name in os.listdir(os.path.join(root, subdir, 'images')):
                image = cv2.imread(os.path.join(
                    root, subdir, 'images', image_name), cv2.IMREAD_UNCHANGED)
                image = cv2.resize(image, size, interpolation=cv2.INTER_LANCZOS4)
                mask = cv2.imread(os.path.join(
                    root, subdir, 'masks', image_name), cv2.IMREAD_UNCHANGED)
                mask[mask!=0] = 1
        ...
        self.masks = torch.tensor(
            np.array(masks, dtype=np.int64), dtype=torch.int64)

    def __getitem__(self, ix):
        return self.images[ix], self.masks[ix]
