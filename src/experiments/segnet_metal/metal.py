...
class MetalDataset(Dataset):
    def __init__(self, image_root, mask_root, start, end):
        ...
        for image_name in sorted(os.listdir(image_root)[start:end]):
            ...
            mask = cv2.imread(os.path.join(
                mask_root, f'RGMask{signature}'), cv2.IMREAD_GRAYSCALE)
            mask[mask < 200] = 0
            mask[mask >= 200] = 1
            for i in range(3):
                for j in range(3):
                    x = image[i*128:(i+1)*128, j*128:(j+1)*128]
                    y = mask[i*128:(i+1)*128, j*128:(j+1)*128]
                    images.append([x / 255.])
                    masks.append(y)
